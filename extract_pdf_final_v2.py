...
def clean_account_name_aggressive(name: str) -> str:
    """Limpa nome da conta removendo números misturados de forma agressiva"""
    if not name:
        return ""
    ...
    # Remover dígitos isolados que estão misturados com letras
    # Estratégia: identificar padrões como "CAIXA4" -> "CAIXA" ou "CIRCU2L1A" -> "CIRCULA"
    result = []
    i = 0
    
    while i < len(name):
        # Verificar se estamos em uma área protegida
        in_protected = any(start <= i < end for start, end in protected_ranges)
        
        if in_protected:
            # Encontrar fim da área protegida
            end = max(end for start, end in protected_ranges if start <= i < end)
            result.append(name[i:end])
            i = end
        else:
            char = name[i]
            # Se é um dígito isolado entre letras, pular
            if char.isdigit():
                # Verificar contexto: se está entre letras, remover
                if i > 0 and i < len(name) - 1:
                    prev_char = name[i-1]
                    next_char = name[i+1]
                    if prev_char.isalpha() and next_char.isalpha():
                        # É um dígito misturado, pular
                        i += 1
                        continue
                # Se é parte de um número maior (2+ dígitos), pode ser código válido
                # Mas se está isolado, remover
                if i == 0 or not name[i-1].isdigit():
                    if i == len(name) - 1 or not name[i+1].isdigit():
                        # Dígito isolado, pular
                        i += 1
                        continue
    ...
    # Limpeza adicional: remover padrões como "CAIXA4" -> "CAIXA"
    cleaned = re.sub(r'([A-Za-zÀ-ÿ])(\d+)(\s|$|[A-Z])', r'\1\3', cleaned)
    
    # Remover dígitos isolados restantes entre letras
    cleaned = re.sub(r'([A-Za-zÀ-ÿ])(\d)([A-Za-zÀ-ÿ])', r'\1\3', cleaned)
...
def parse_data_line(line: str) -> Optional[Dict[str, Any]]:
    ...
    if not monetary_matches:
        ...
        if len(line) > 10:
            cleaned_account = clean_account_name_aggressive(line)
    ...
    # Procurar código (4-6 dígitos, não classificação, não parte de valores monetários)
    ...
    account = clean_account_name_aggressive(account_text)
                ...
                if line_y is not None:
                    ...
                    for bold_y, bold_text in bold_categories.items():
                        if abs(y_normalized - bold_y) < 5:
                            ...
                            cleaned_bold = clean_account_name_aggressive(bold_text)
